          ...
          <div class="row">
              <div class="col">
                  <div class="card-body" style="padding:0">
                    <h2 class="text-center pb-3">Reservation Report</h2>
                    <h3 class="text-center pb-3">{{\Carbon\Carbon::parse($date['start'])->format('d/M/Y')}} - {{\Carbon\Carbon::parse($date['end'])->format('d/M/Y')}}</h3>
                    <br>
                    @php
                        if($totals['reservationStatus'] == -2)
                        $statustxt = "Cancel";
                        elseif($totals['reservationStatus'] == -1)
                        $statustxt = "Waiting for Payment";
                        elseif($totals['reservationStatus'] ==0)
                        $statustxt = "Upcoming Reservation";
                        elseif($totals['reservationStatus'] ==1)
                        $statustxt = "Not Coming";
                        else
                        $statustxt = "Completed";
                    @endphp
                    Reservation Status : {{$statustxt}}<br><br>
                    <table class='table table-striped table-bordered'>
                        ...
                        @foreach($data as $u)
                        @php
                          if($u->payment_status == 0 || $u->payment_status == 1)
                            $multiple = 0.5;
                        ...
                        <tr>
                          <td>{{$loop->iteration}}</td>
                            <td width="20%">{{$u->reservation_code}}</td>
                            <td width="20%">{{$u->name}}</td>
                            <td>{{$u->reservation_datetime}}</td>
                            <td>{{$status}}</td>
